...
- (void)prepareLayout
{
    ...
    for (NSInteger section = 0; section < numberOfSections; section++) {
        ...
        for (NSInteger i = 0; i < itemCount; i++) {
            ...
            if (indexPath.row == 0) {
                 attributes.frame = CGRectMake(x, [columnHeights[columnIndex] floatValue] + _ofSetHeight, size.width, size.height);
            }
...
- (void)setOfSetHeight:(CGFloat)ofSetHeight{
    _ofSetHeight = ofSetHeight;
    [self setup];
}
@end
